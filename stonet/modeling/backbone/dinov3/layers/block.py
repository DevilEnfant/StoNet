...
try:
    ...
    # torch._dynamo 在当前PyTorch版本中不可用，跳过配置
    pass
...
class SelfAttentionBlock(nn.Module):
    ...
    def _maybe_index_rope(self, rope: Optional[Tuple[Tensor, Tensor]], indices: Tensor) -> Optional[Tuple[Tensor, Tensor]]:
        if rope is None:
            return None
...
class CausalSelfAttentionBlock(nn.Module):
    ...
    def init_weights(
        ...
        init_fc_std: Optional[float] = None, factor: float = 1.0
    ) -> None:
